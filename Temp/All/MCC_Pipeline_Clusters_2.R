...
for(i in 1:length(contrasts)) {
  ...
  bump.design           <- design
  bump.samples          <- contrasts[i]
  # bump.samples.1        <- bump.samples[1]
  # bump.samples.2        <- bump.samples[2]
  
  myannotation       <- cpg.annotate(minfi::getM(lumi.norm.sub), 
  ...
  for(j in 1:print_cut) { 
    ...
    df                    <- cbind(df,
                                   pData(lumi.norm.sub)[match(df$Var2, 
