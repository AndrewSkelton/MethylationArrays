...
mAdj.fit             <- fitA$coefficients[,-c(1:3)]
mAdj                 <- as.matrix(mAdj.in) - mAdj.fit %*% t(design[,-c(1:3)])
...
for(i in 1) {
  for(k in c("Source", "SampleType", "Gender")) {
    j  <- i+1
    ...
    #                   hjust       = -0.2)                  +
            theme_bw()                                     +
            ggtitle("PCA of Raw Methylation Data")  +
            theme(axis.title.x    = element_text(size=15),
                  axis.title.y    = element_text(size=15)) +
            xlab(label=paste0("PC", i, "(", pcv[i], "%)"))      +
            ylab(label=paste0("PC", j, "(", pcv[j], "%)"))
    
    png(paste0("afnorm_PCA_all_", k, "_", i, j, ".png"),
        width  = 12.53, 
        height = 6.98, 
        units  = "in", 
        res    = 600)
...
##'Beta Density
##'-----------------------------------------------------------------------------------------#
png("Norm_BetaDist_Q.png",
    width  = 12.53, 
    height = 6.98, 
    units  = "in", 
    res    = 600)
densityPlot(getBeta(lumi.norm.base.q), #lumi.norm.base
            sampGroups = pData(lumi.norm.base)$Source,
            main       = "Beta Distribution Quantile", 
            xlab       = "Beta")
dev.off()
##'-----------------------------------------------------------------------------------------#
...
##'Gender Prediction
##'-----------------------------------------------------------------------------------------#
comp_sex      <- as.data.frame(getSex(lumi.norm))
comp_sex      <- as.data.frame(cbind(comp_sex, 
                                     pData(lumi.norm)))
ggplot(comp_sex, 
       aes(x            = xMed, 
           y            = yMed)) + 
  geom_point(aes(colour = Gender)) + 
  geom_text(aes(label   = comp_sex$Sample.ID, 
                vjust   = 1.2, 
                hjust   = -0.2)) +
  theme_bw()
##'-----------------------------------------------------------------------------------------#