...
#  Description : Linear Variable Covariate Model - A Tad more Complex - mAge-cAge Regression|
#-------------------------------------------------------------------------------------------#

current_dir <- getwd()
dir.create(paste0(current_dir,
                  "/mAge-cAge_Regression_Advanced_Model"))
...
mcAge       <- as.numeric(pData(lvcm.in)$mcAgeDiff)
Data_Source <- factor(pData(lvcm.in)$Source)
lvcm.design <- model.matrix(~SampleType:mcAge + Data_Source)
# colnames(lvcm.design)
colnames(lvcm.design)[6:10] <- levels(SampleType)
...
lvcm.fit    <- contrasts.fit(lvcm.fit, contrasts = cont_mat)
...
# }
# 
#   assign(paste0(gsub(":mcAge", 
#                      "",
#                      colnames(lvcm.design)[i])), 
#          lvcm.filter)
# }
# save(SampleTypeLesioned_OA_Hip,       SampleTypeLesioned_OA_Knee, 
#      SampleTypePreserved_Control_Hip, SampleTypePreserved_OA_Hip, 
#      SampleTypePreserved_OA_Knee,     file = "mcAge_Lists.RData")
  
  assign(lvcm.name, 
         lvcm.filter)
}
save(all, oa_only, hip_only, knee_only, file = "mcAge_Collapse_Lists.RData")
  ...
  if(!is.null(nrow(beta_out))) {
    ...
    write.csv(beta_out, 
              file=paste0(lvcm.name,
                          "_mAge-cAge_AdjustedBetas_top100.csv"))
    ##'-----------------------------------------------------------------------------------------#
