...
get_age_reg <- function(lvcm_sample, lvcm_in) {
  ...
  if(length(levels(factor(pData(lvcm_in)$Source))) > 1) {
    ...
    mAdj_fit_age    <- lvcm_fit2$coefficients[,-c(1:2)]
    mAdj_age        <- as.matrix(mAdj_in_age) - mAdj_fit_age %*% t(lvcm_design[,-c(1:2)])
...
# Fit Model
lvcm_sample   <- "All"
oa_all        <- get_age_reg(lvcm_sample, norm_data_f)
```
